...
def generate_code(
    ...
    canon_solver = None
    canon, solver_interface = extract_canonicalization(problem, solver, solver_opts)
    ...
    solver_interface.generate_code(
        configuration,
        code_dir,
        solver_code_dir,
        cvxpygen_directory,
        canon.parameter_canon,
        configuration.prefix,
    )
    write_c_code(problem, configuration, canon, canon_solver, solver_interface)

    sys.stdout.write("CVXPYgen finished generating code.\n")


def extract_canonicalization(problem, solver, solver_opts) -> Canon:
    # problem data
    ...
    dual_variable_info = get_dual_variable_info(
        inverse_data, solver_interface, cvxpy_interface_class
    )
    ...
    return Canon(
        prim_variable_info, dual_variable_info, parameter_info, parameter_canon
    ), solver_interface
...
def merge_canonicalizations(canon_first, canon_second, solver_interface) -> Canon:
    ...
    pc.p_id_to_changes = (
        pc_second.p_id_to_changes
    )  # TODO: consider cases where q, A, l, or u do not change
    ...
    map_first.append(create_constant_map(1, n_user_param, 1.0))
...
def get_quad_obj(problem, solver_type, solver_opts, solver_class) -> bool:
    ...
    return (
        use_quad_obj
        and solver_class().supports_quad_obj()
...
def process_canonical_parameters(
    ...
    for i, p_id in enumerate(canon_p_ids):
        ...
        if affine_map:
            ...
            adjacency = update_adjacency_matrix(
            ...
            affine_map, parameter_canon = set_default_values(
...
def set_default_values(
    ...
    if p_id.isupper():
        ...
        if solver_interface.dual_var_split:  # by equality and inequality
            ...
            affine_map.indptr = (
                0 * indptr_original
            )  # rebuild 'indptr' by considering only 'mapping_rows' (corresponds to either equality or inequality)
            for r in affine_map.mapping_rows:
                for c in range(
                    affine_map.shape[1]
                ):  # shape of matrix re-shaped from flat param vector resulting from mapping
            ...
            if p_id == "P":
                ...
                affine_map.indptr = solver_interface.indptr_constr[
                    :-1
                ]  # leave out part for rhs
        ...
        parameter_canon.p[p_id] = solver_interface.augment_vector_parameter(
            p_id, affine_map.mapping @ parameter_info.flat_usp
        )
...
def get_primal_variable_info(problem, inverse_data) -> PrimalVariableInfo:
    ...
    var_name_to_indices = {}
    for var_name, offset, shape, sym in zip(
        var_names, var_offsets, var_shapes, var_sym
    ):
        if sym:
...
def get_dual_variable_info(
    ...
    if solver_interface.solver_type == "quadratic":
        con_canon = inverse_data[
            -2
        ].constraints  # same order as in canonical dual vector
    elif solver_interface.solver_type == "conic":
        con_canon = (
            inverse_data[-1][cvxpy_interface_class.EQ_CONSTR]
            + inverse_data[-1][cvxpy_interface_class.NEQ_CONSTR]
        )
    ...
    if solver_interface.dual_var_split:
        ...
        d_vectors = [solver_interface.dual_var_names[0]] * n_split + [
            solver_interface.dual_var_names[1]
        ] * (len(d_canon_offsets) - n_split)
    ...
    d_name_to_indices = {
        n: (v, o + np.arange(np.prod(d_name_to_shape[n])))
...
def get_constraint_info(solver_interface) -> ConstraintInfo:
    ...
    mapping_rows_eq = np.nonzero(
        solver_interface.indices_constr < solver_interface.n_eq
    )[0]
    mapping_rows_ineq = np.nonzero(
        solver_interface.indices_constr >= solver_interface.n_eq
    )[0]

    return ConstraintInfo(
        n_data_constr, n_data_constr_mat, mapping_rows_eq, mapping_rows_ineq
    )
...
def write_c_code(
    ...
    write_file(
        os.path.join(include_dir, f"{configuration.prefix}workspace.h"),
        "w",
    ...
    write_file(
        os.path.join(src_dir, f"{configuration.prefix}workspace.c"),
        "w",
    ...
    write_file(
        os.path.join(include_dir, f"{configuration.prefix}solve.h"),
        "w",
        getattr(utils, "write_solve_prot"),
        configuration,
        prim_variable_info,
        dual_variable_info,
        parameter_info,
        parameter_canon,
        solver_interface,
        parameter_canon_gradient,
    )

    write_file(
        os.path.join(src_dir, f"{configuration.prefix}solve.c"),
        "w",
        getattr(utils, "write_solve_def"),
        configuration,
        prim_variable_info,
        dual_variable_info,
        parameter_info,
    ...
    write_file(
        os.path.join(cpp_dir, "include", f"{configuration.prefix[:-1]}.hpp"),
        "w",
        write_module_prot,
        configuration,
        parameter_info,
    ...
    write_file(
        os.path.join(cpp_dir, "src", f"{configuration.prefix[:-1]}.cpp"),
        "w",
        write_module_def,
        configuration,
        prim_variable_info,
    ...
    write_file(
        os.path.join(configuration.code_dir, f"{configuration.prefix[:-1]}.py"),
        "w",
        write_method,
        configuration,
        prim_variable_info,
    ...
    write_file(
        os.path.join(configuration.code_dir, "problem.pickle"),
        "wb",
    ...
    read_write_file(
        os.path.join(c_dir, "CMakeLists.txt"), replace_cmake_data, configuration
    )

    read_write_file(
        os.path.join(configuration.code_dir, "setup.py"), replace_setup_data
    )

    read_write_file(
        os.path.join(configuration.code_dir, "README.html"),
        replace_html_data,
        configuration,
        prim_variable_info,
        dual_variable_info,
...
def get_configuration(
    code_dir, solver_name, unroll, prefix, gradient, gradient_two_stage
) -> Configuration:
...
def get_parameter_info(p_prob) -> ParameterInfo:
    ...
    for p in p_prob.parameters:
        if p.attributes["sparsity"]:
            ...
            user_p_name_to_sparsity[p.name()] = np.sort(
                [r + p.shape[0] * c for r, c in zip(*p.attributes["sparsity"])]
            )
            ...
            user_p_sparsity_mask[
                user_p_id_to_col[p.id] : user_p_id_to_col[p.id]
                + user_p_id_to_size[p.id]
            ] = False
            user_p_sparsity_mask[
                user_p_id_to_col[p.id] + user_p_name_to_sparsity[p.name()]
            ] = True
    ...
    user_p_flat = cI.get_parameter_vector(
    ...
    for param in p_prob.parameters:
        ...
        if param.attributes["sparsity"]:
            if param.size == 1 or max(param.shape) == param.size:
                ...
                warnings.warn(
                    f"Ignoring sparsity pattern for {param_type} parameter {param.name()}!"
                )
                ...
                for r, c in zip(*param.attributes["sparsity"]):
                    if r < 0 or c < 0 or r >= param.shape[0] or c >= param.shape[1]:
                        warnings.warn(
                            f"Invalid sparsity pattern for parameter {param.name()} - out of range! Ignoring sparsity pattern."
                        )
                        param.attributes["sparsity"] = None
        ...
        if param.attributes["sparsity"] and param.value is not None:
            for i in range(param.shape[0]):
                for j in range(param.shape[1]):
                    if (i, j) not in zip(*param.attributes["sparsity"]) and param.value[
                        i, j
                    ] != 0:
                        warnings.warn(
                            f"Ignoring nonzero value outside of sparsity pattern for parameter {param.name()}!"
                        )
                        param.value[i, j] = 0
...
def create_folder_structure(code_dir: str):
    ...
    # Copy template files
    shutil.copy(
        os.path.join(cvxpygen_directory, "template", "CMakeLists.txt"),
