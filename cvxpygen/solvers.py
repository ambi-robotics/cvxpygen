...
from cvxpygen.mappings import (
    ...
    DualVariableInfo,
    ParameterCanon,
    ParameterUpdateLogic,
    PrimalVariableInfo,
    UpdatePendingLogic,
    WorkspacePointerInfo,
)
from cvxpygen.utils import multiple_replace, read_write_file
...
class SolverInterface(ABC):
    ...
    def get_affine_map(
        self, p_id, param_prob, constraint_info: ConstraintInfo
    ) -> AffineMap:
        ...
        if p_id in ["A", "b"]:
            ...
            affine_map.indices = (
                self.indices_constr[affine_map.mapping_rows] - self.n_eq
            )
    ...
    def stgs_names_to_type(self):
        return dict(zip(self.stgs_names, self.stgs_types, strict=True))

    @property
    def stgs_names_to_default(self):
        return dict(zip(self.stgs_names, self.stgs_defaults, strict=True))
...
class ClarabelInterface(SolverInterface):
    ...
    def __init__(self, data, p_prob, enable_settings):
        ...
        indices_obj, indptr_obj, shape_obj = self.get_problem_data_index(
            p_prob.reduced_P
        )
        indices_constr, indptr_constr, shape_constr = self.get_problem_data_index(
            p_prob.reduced_A
        )
        ...
        canon_constants["n_cone_types"] = (
            int(p_prob.cone_dims.zero > 0)
        ...
        self.parameter_update_structure = {
            "init": ParameterUpdateLogic(
                update_pending_logic=UpdatePendingLogic(
                    ...
                    functions_if_false=update_after_init,
                ),
                function_call=f"{{prefix}}copy_all();\n"
    ...
    def generate_code(
        ...
        if is_sdp:
            sys.stdout.write(
                "WARNING: You are generating code for an SDP with Clarabel, which requires BLAS and LAPACK within Rust-C wrappers - expect large compilation time and binary size.\n"
            )
        ...
        for dtc in dirs_to_copy:
            shutil.copytree(
                os.path.join(cvxpygen_directory, "solvers", "Clarabel.cpp", dtc),
                os.path.join(solver_code_dir, dtc),
            )
        files_to_copy = ["CMakeLists.txt", "LICENSE.md"]
        for fl in files_to_copy:
            shutil.copyfile(
                os.path.join(cvxpygen_directory, "solvers", "Clarabel.cpp", fl),
        ...
        with open(os.path.join(code_dir, "c", "CMakeLists.txt"), "a") as f:
            if is_sdp:
                ...
                link_libraries = (
                    "libclarabel_c_static ${BLAS_LIBRARIES} ${LAPACK_LIBRARIES}"
                )
            else:
        ...
        read_write_file(
            os.path.join(code_dir, "c", "solver_code", "CMakeLists.txt"),
        ...
        if is_sdp:
            read_write_file(
                os.path.join(code_dir, "c", "solver_code", "CMakeLists.txt"),
                lambda x: x.replace(
                    'set(CLARABEL_FEATURE_SDP "none"',
                    'set(CLARABEL_FEATURE_SDP "sdp-openblas"',
                ),
            )
        ...
        read_write_file(
            os.path.join(
                code_dir, "c", "solver_code", "rust_wrapper", "CMakeLists.txt"
            ),
            lambda x: multiple_replace(x, replacements),
        )
        ...
        # adjust Clarabel
        read_write_file(
            os.path.join(code_dir, "c", "solver_code", "include", "Clarabel"),
    ...
    def declare_workspace(self, f, prefix, parameter_canon) -> None:
        ...
        f.write(
            f"extern ClarabelSupportedConeT {prefix}cones[{self.canon_constants['n_cone_types']}];\n"
        )
    ...
    def define_workspace(self, f, prefix, parameter_canon) -> None:
        ...
        for cd in ["cone_dims_zero", "cone_dims_nonneg"]:
            if self.canon_constants[cd] > 0:
                cone_str_list.append(
                    f"{self.canon_constants['cd_to_t'][cd]}({self.canon_constants[cd]})"
                )
        cone_str_list.extend(
            ["ClarabelExponentialConeT()"] * self.canon_constants["cone_dims_exp"]
        )
        ...
        f.write(
            f"ClarabelSupportedConeT {prefix}cones[{self.canon_constants['n_cone_types']}] = {{ {', '.join(cone_str_list)} }};\n"
        )
...
def get_interface_class(
    solver_name: str,
) -> tuple[type[SolverInterface], type[ConicSolver]]:
    mapping = {"CLARABEL": (ClarabelInterface, CLARABEL)}
